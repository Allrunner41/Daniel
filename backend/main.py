...
HF_MODEL = os.environ.get("HF_MODEL", "Hello-SimpleAI/chatgpt-detector-roberta")
...
# (curl, Postman, page de test locale).
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def _parse_hf_output(raw: object) -> Tuple[float, float]:
    """
    Normalise la sortie du modèle Hugging Face — dont le format varie selon
    le modèle utilisé — en (probabilité_ia, probabilité_humain).
    """
    scores = raw
    ...
    for item in scores:
        ...
        if any(k in label for k in ("fake", "generated", "chatgpt", "gpt", "ai")):
            ai_prob = score
        elif any(k in label for k in ("real", "human")):
            human_prob = score
    ...
    if ai_prob is None or human_prob is None:
        logger.error("Impossible d'associer les labels du modèle à IA/Humain.")
        raise HTTPException(
